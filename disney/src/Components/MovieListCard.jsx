          ...
          list.map((movie, index) => (
            <div key={index} className='flex-shrink-0 '>
              <img
                ...
                alt={movie.title}
                className='w-[150px] sm:w-[300px] my-10  rounded-lg hover:scale-105 transition-all ease-in-out p-1 hover:border-[5px] border-gray-400'
              />
            </div>
          ))
        ) : (
          <p className='text-white'>No movies available for this genre.</p>
        )}
      </div>
    </div>
  )
